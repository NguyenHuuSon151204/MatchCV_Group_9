...
ALTER TABLE dbo.RewriteSuggestions
ADD CONSTRAINT FK_RewriteSuggestions_MatchRuns
    FOREIGN KEY (MatchId)
    REFERENCES dbo.MatchRuns(Id)
    ON DELETE CASCADE;          -- giữ lại: xóa MatchRun thì xóa Suggestion

ALTER TABLE dbo.RewriteSuggestions
ADD CONSTRAINT FK_RewriteSuggestions_Bullets
    FOREIGN KEY (BulletId)
    REFERENCES dbo.Bullets(Id)
    ON DELETE SET NULL;         -- ***QUAN TRỌNG***: KHÔNG dùng CASCADE
GO
